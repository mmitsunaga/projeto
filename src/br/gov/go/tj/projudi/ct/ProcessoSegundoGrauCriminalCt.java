 ...
 * @author msapaula
 */
public class ProcessoSegundoGrauCriminalCt extends ProcessoComumCt {
	...
	public void executar(HttpServletRequest request, HttpServletResponse response, UsuarioNe UsuarioSessao, int paginaatual, String tempNomeBusca, String PosicaoPaginaAtual) throws Exception, ServletException, IOException {
		...
		request.setAttribute("tempRetorno", "ProcessoSegundoGrauCriminal");
		...
		switch (paginaatual) {
			...
			case Configuracao.SalvarResultado:
				...
				if (UsuarioSessao.getUsuarioDt().getServentiaTipoCodigo() != null && UsuarioSessao.getUsuarioDt().getServentiaTipoCodigo().trim().equalsIgnoreCase(String.valueOf(ServentiaTipoDt.DELEGACIA)))
					processoDt.setId_ServentiaOrigem(UsuarioSessao.getUsuarioDt().getId_Serventia());
				
				Mensagem = Processone.verificarProcessoSegundoGrauCriminal(processoDt, UsuarioSessao.getUsuarioDt());

				if (Mensagem.length() == 0) {
					Processone.cadastrarProcessoSegundoGrau(processoDt, UsuarioSessao.getUsuarioDt());
			...
			case Configuracao.Localizar:
				...
				if (processoNumeroVinculo != null && processoNumeroVinculo.length() > 0) {
							...
							if (processoNumeroVinculo.indexOf(".") > 0) {
								...
								} else request.setAttribute("MensagemErro", "Não foi possível localizar o Processo informado.");
							} else request.setAttribute("MensagemErro", "Número do Processo no formato incorreto. ");
				}
			break;
			...
			case (AreaDistribuicaoDt.CodigoPermissao * Configuracao.QtdPermissao + Configuracao.Localizar):				
				...
				if (request.getParameter("Passo")==null){
					...
					String stTemp = "";
					stTemp =Processone.consultarAreasDistribuicaoSegundoGrauCriminalJSON(stNomeBusca1, processoDt.getId_Comarca(), PosicaoPaginaAtual, turmaJulgadora);
			...
			case (AssuntoDt.CodigoPermissao * Configuracao.QtdPermissao + Configuracao.Localizar):
				if (request.getParameter("Passo")==null){
					...
					if (processoDt.isProcessoDependente()){
						if (processoDt.getProcessoDependenteDt().getId_ServentiaOrigemRecurso() != null && processoDt.getProcessoDependenteDt().getId_ServentiaOrigemRecurso().length()>0 ){							
			...
			case (EstadoCivilDt.CodigoPermissao * Configuracao.QtdPermissao + Configuracao.Localizar):
				if (request.getParameter("Passo")==null){
					...
					atribuirJSON(request, "Id_EstadoCivil", "EstadoCivil", "EstadoCivil", "ProcessoSegundoGrauCriminal", Configuracao.Editar, stPemissao, lisNomeBusca, lisDescricao);
					passoEditar = 4;
				}else{
					String stTemp = "";
			...
			default:
				...
				switch (passoEditar) {
					...
					case 9:
						...
						request.setAttribute("tempRetorno", "ProcessoSegundoGrauCriminal?PassoEditar=3&ParteTipo=" + parteTipo);
						break;
					...
					case 11:
						redireciona(response, "ProcessoCadastroRestaurar?tempRetorno=ProcessoSegundoGrauCriminal");
						break;
