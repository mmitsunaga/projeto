...
import br.gov.go.tj.projudi.ne.GuiaExecucaoQueixaCrimeNe;
...
public class GuiaExecucaoQueixaCrimeCt extends Controle {

	private static final long serialVersionUID = 5522659051373116998L;
	
	private static final String PAGINA_GUIA_EXECUCAO_QUEIXA_CRIME 	= "/WEB-INF/jsptjgo/GuiaExecucaoQueixaCrime.jsp";
	...
	private GuiaExecucaoQueixaCrimeNe guiaExecucaoQueixaCrimeNe = new GuiaExecucaoQueixaCrimeNe();
	...
	public void executar(HttpServletRequest request, HttpServletResponse response, UsuarioNe UsuarioSessao, int paginaatual, String nomebusca, String posicaopaginaatual) throws Exception, ServletException, IOException {
		...
		stAcao = PAGINA_GUIA_EXECUCAO_QUEIXA_CRIME;
		...
		request.getSession().setAttribute("visualizarFieldsetAdicionarLocomocao", guiaExecucaoQueixaCrimeNe.podeEmitirLocomocao(processoDt.getId(), processoDt.getProcessoTipoCodigo(), GuiaTipoDt.ID_FINAL_EXECUCAO_QUEIXA_CRIME));
		
		//********************************************
		//Requests 
		request.setAttribute("tempPrograma" 			, "Guia Final(Exec. Sentença Queixa-Crime)");
		request.setAttribute("tempRetorno" 				, "GuiaExecucaoQueixaCrime");
		...
		if( request.getParameter("atoEscrivao") != null && !(request.getParameter("atoEscrivao").equals("null")) && request.getParameter("atoEscrivao").toString().length() > 0 ) {
		...
		if( request.getParameter("contadorQuantidadeAcrescimo") != null && request.getParameter("contadorQuantidadeAcrescimo").toString().length() > 0 ) {
			guiaEmissaoDt.setContadorQuantidadeAcrescimo(request.getParameter("contadorQuantidadeAcrescimo").toString());
		}
		...
		if( processoDt != null && processoDt.getServentiaCodigo() != null ) {
			serventiaDt = guiaExecucaoQueixaCrimeNe.consultarServentiaProcesso(processoDt.getServentiaCodigo());
			if( serventiaDt == null ) {
				serventiaDt = guiaExecucaoQueixaCrimeNe.consultarIdServentia(processoDt.getId_Serventia());
			}
			if( serventiaDt != null ) {
				comarcaDt = guiaExecucaoQueixaCrimeNe.consultarComarca(serventiaDt.getId_Comarca());
		...
		if( request.getParameter("rateioCodigo") != null && ( Funcoes.StringToInt(request.getParameter("rateioCodigo")) == guiaExecucaoQueixaCrimeNe.RATEIO_VARIAVEL || Funcoes.StringToInt(request.getParameter("rateioCodigo")) == guiaExecucaoQueixaCrimeNe.RATEIO_50_50 )  ) {
		...
		if( guiaEmissaoDt.getRateioCodigo() != null && guiaEmissaoDt.getRateioCodigo().length() > 0 && Funcoes.StringToInt(guiaEmissaoDt.getRateioCodigo()) == GuiaExecucaoQueixaCrimeNe.RATEIO_VARIAVEL ) {
		...
		switch(paginaatual) {
			...
			case Configuracao.Novo: {
				...
				if( guiaExecucaoQueixaCrimeNe.isServentiaSegundoGrau(processoDt.getId_Serventia()) ) {
				...
				if( !guiaExecucaoQueixaCrimeNe.isProcessoSegundoGrau(serventiaDt.getId()) ) {
					...
					if( conectouSPG ) {
						...
						guiaExecucaoQueixaCrimeNe.tratamentoParaProcessosSigilosoSegredoJustica(processoDt);
						...
						request.getSession().setAttribute("ListaCustaDt", guiaExecucaoQueixaCrimeNe.consultarItensGuia(null, GuiaTipoDt.ID_FINAL_EXECUCAO_QUEIXA_CRIME, processoDt.getId_ProcessoTipo()));
						...
						guiaEmissaoDt.setNovoValorAcao( processoDt.getValor() );
						guiaEmissaoDt.setNovoValorAcaoAtualizado( Funcoes.FormatarDecimal(guiaExecucaoQueixaCrimeNe.atualizarValorCausaUFR(processoDt.getValor(), Funcoes.StringToDate(processoDt.getDataRecebimento())).toString()) );
						...
						if( guiaExecucaoQueixaCrimeNe.existeGuiaEmitidaMesmoTipo(processoDt.getId(), GuiaTipoDt.ID_FINAL_EXECUCAO_QUEIXA_CRIME) ) {
			...
			case Configuracao.Atualizar : {
				...
				guiaEmissaoDt.setNovoValorAcaoAtualizado( Funcoes.FormatarDecimal(guiaExecucaoQueixaCrimeNe.atualizarValorCausaUFR(guiaEmissaoDt.getNovoValorAcao(), Funcoes.StringToDate(guiaEmissaoDt.getDataBaseFinalAtualizacao())).toString()) );
			...
			case Configuracao.Curinga6 : {
				switch(passoEditar) {
					...
					case Configuracao.Curinga7 : {
						...
						if( guiaEmissaoDt.getRateioCodigo().equals(String.valueOf(GuiaEmissaoNe.RATEIO_VARIAVEL)) ) {
							request.setAttribute("visualizaDivRateioPartesVariavel", "DivVisivel");
						}
						else {
							List listaPromoventes = processoDt.getListaPolosAtivos();
					...
					case Configuracao.Curinga8: {
						...
						listaGuiasRateio = new ArrayList();
						listaTotalGuiaRateio = new ArrayList();
						listaNomeParteGuia = new ArrayList();
						listaNomePartePorcentagemGuia = new ArrayList();
						if( Funcoes.StringToInt(request.getParameter("rateioCodigo").toString()) == GuiaExecucaoQueixaCrimeNe.RATEIO_VARIAVEL ) {
...
//										totalRateio += Funcoes.StringToDouble(VARIAVEL_RATEIO_PARTE_VARIAVEL);
							...
							guiaEmissaoDt.setTotalRateio(totalRateio.doubleValue());
						}
						
						//Consulta lista de itens da guia
						//listaItensGuia = guiaFinalNe.consultarItensGuia(guiaEmissaoDt, GuiaTipoDt.EXECUCAO_QUEIXA_CRIME, processoDt.getId_ProcessoTipo());
						listaItensGuia = null;
						
						if( listaItensGuia == null ) {
							GuiaModeloDt guiaModeloDt = new GuiaModeloNe().consultarGuiaModeloProcessoTipo(null, GuiaTipoDt.ID_FINAL_EXECUCAO_QUEIXA_CRIME, processoDt.getId_ProcessoTipo());
							guiaEmissaoDt.setGuiaModeloDt(guiaModeloDt);
						}
						
						if( listaCustaDt != null && listaCustaDt.size() > 0 ) {
							List listaAux = guiaExecucaoQueixaCrimeNe.consultarItensGuiaCustaDt(guiaEmissaoDt, listaCustaDt);
							if( listaAux != null ) {
								if( listaItensGuia == null )
									listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
						...
						if( listaBairroDt != null && listaBairroDt.size() > 0 ) {
							...
							for(int i = 0; i < listaBairroDt.size(); i++) {
								...
								for(int j = 0; j < quantidadeLocomocao; j++) {
									listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItemLocomocaoOficial(bairroLocomocaoDt.getBairroDt(), bairroLocomocaoDt.getOficialSPGDt()) );
								}
							}
						}
						...
						if( listaBairroLocomocaoContaVinculada != null && listaBairroLocomocaoContaVinculada.size() > 0 ) {
							...
							for(int i = 0; i < listaBairroLocomocaoContaVinculada.size(); i++) {
								...
								for(int j = 0; j < quantidadeLocomocao; j++) {
									listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItemLocomocaoOficialContaVinculada(bairroLocomocaoDt.getBairroDt(), bairroLocomocaoDt.getOficialSPGDt()) );
								}
							}
						}
						...
						//Atenï¿½ï¿½o
						valoresReferenciaCalculo.put(CustaDt.GUIA_EXECUCAO_QUEIXA_CRIMINE, "SIM");
						...
						if( guiaEmissaoDt.getContadorQuantidade() != null && guiaEmissaoDt.getContadorQuantidade().length() > 0 && !guiaEmissaoDt.getContadorQuantidade().equals("0") ) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.CONTADOR) );
						}
						...
						if( guiaEmissaoDt.getContadorQuantidadeAcrescimo() != null && guiaEmissaoDt.getContadorQuantidadeAcrescimo().length() > 0 && !guiaEmissaoDt.getContadorQuantidadeAcrescimo().equals("0") ) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.PARA_REALIZACAO_E_CONFERENCIA_DE_CALCULOS_E_ATRIBUICOES_10_MINIMO_ITEM_5) );
						}
						
						//Atos dos Escrivï¿½es do Crime
						if( guiaEmissaoDt.getAtoEscrivao() != null && guiaEmissaoDt.getAtoEscrivao().toString().length() > 0 && !guiaEmissaoDt.getAtoEscrivao().equals("0") ) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.AUTUACAO_E_OU_PROCESSAMENTO_DE_FEITOS) );
						...
						if( guiaEmissaoDt.getTaxaProtocoloQuantidade() != null && guiaEmissaoDt.getTaxaProtocoloQuantidade().length() > 0 && !guiaEmissaoDt.getTaxaProtocoloQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.TAXA_PROTOCOLO) );
						}
						...
						if( guiaEmissaoDt.getCorreioQuantidade() != null && guiaEmissaoDt.getCorreioQuantidade().length() > 0 && !guiaEmissaoDt.getCorreioQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.DESPESA_POSTAL) );
						}
						...
						if( guiaEmissaoDt.getDepositarioPublico() != null && guiaEmissaoDt.getDepositarioPublico().length() > 0 && !guiaEmissaoDt.getDepositarioPublico().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.DEPOSITO_COMPREENDENDO_OS_REGISTROS_GUARDA_ESCRITURACAO) );
						}
						...
						if( guiaEmissaoDt.getAvaliadorQuantidade() != null && guiaEmissaoDt.getAvaliadorQuantidade().length() > 0 && guiaEmissaoDt.getAvaliadorValor().length() > 0 && !guiaEmissaoDt.getAvaliadorQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.AUTO_DE_AVALIACAO_DE_BENS_EM_PROCESSO_DE_QUALQUER_NATUREZA) );
						}
						...
						if( guiaEmissaoDt.getLeilaoQuantidade().length() > 0 && guiaEmissaoDt.getLeilaoValor().length() > 0 && !guiaEmissaoDt.getLeilaoQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.PREGAO_PRACAO_LEILAO) );
						}
						...
						if( guiaEmissaoDt.getPartidorQuantidade().length() > 0 && !guiaEmissaoDt.getPartidorQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.CUSTA_PARTIDOR) );
						}
						...
						if( guiaEmissaoDt.getDocumentoDiarioJustica() != null && guiaEmissaoDt.getDocumentoDiarioJustica().length() > 0 && !guiaEmissaoDt.getDocumentoDiarioJustica().equals("0")) {
							...
							listaItensGuia.add( guiaExecucaoQueixaCrimeNe.adicionarItem(CustaDt.POR_DOCUMENTO_PUBLICADO) );
						}
						
						
						
						//Custas 30% Contenciosa
//						guiaExecucaoQueixaCrimeNe.verificarCobranca30PorcentoCustas(guiaEmissaoDt, listaItensGuia);
						
						guiaEmissaoDt.setId_Processo(processoDt.getId_Processo());
						List listaGuiaItemDt = new ArrayList();
						listaGuiaItemDt = guiaExecucaoQueixaCrimeNe.calcularItensGuia(guiaEmissaoDt, listaItensGuia, valoresReferenciaCalculo, null, null);
						...
						if( listaGuiaItemDt != null &&
							...
							for( int i = 0; i < listaNomeParteGuia.size(); i++ ) {
								List listaRateioGuiaItemDt = guiaExecucaoQueixaCrimeNe.calcularItensGuiaRateio(guiaEmissaoDt, listaGuiaItemDt, Funcoes.StringToDouble(listaNomePartePorcentagemGuia.get(i).toString()));
							...
							if( listaGuiasRateio.size() > 0 ) {
								for( int m = 0; m < listaGuiasRateio.size(); m++ ) {
									listaTotalGuiaRateio.add(guiaExecucaoQueixaCrimeNe.calcularTotalGuia((List)listaGuiasRateio.get(m)));
								}
						...
						if( listaGuiaItemDt != null && listaGuiaItemDt.size() > 0 ) {
							...
							if( apresentaPrevia ) {
								...
								request.setAttribute("TotalGuia", guiaExecucaoQueixaCrimeNe.getGuiaCalculoNe().getTotalGuia() );
								
								request.getSession().setAttribute("ListaGuiaItemDt" 			, listaGuiaItemDt);
								request.getSession().setAttribute("TotalGuia" 					, Funcoes.FormatarDecimal( guiaExecucaoQueixaCrimeNe.getGuiaCalculoNe().getTotalGuia().toString() ) );
								request.setAttribute("visualizarBotaoImpressaoBotaoPagamento"	, guiaExecucaoQueixaCrimeNe.visualizarBotaoImpressaoBotaoPagamento(guiaEmissaoDt));
...
//					guiaEmissaoDt.setNumeroGuiaCompleto( guiaExecucaoQueixaCrimeNe.getNumeroGuiaCompleto(guiaEmissaoDt.getNumeroGuia()) );
//				}
...
//					guiaEmissaoDt2.setNumeroGuiaCompleto( guiaExecucaoQueixaCrimeNe.getNumeroGuiaCompleto(guiaEmissaoDt2.getNumeroGuia()) );
//					
//					guiaExecucaoQueixaCrimeNe.calcularRateio50Porcento(guiaEmissaoDt.getListaGuiaItemDt());
//					guiaExecucaoQueixaCrimeNe.calcularRateio50Porcento(guiaEmissaoDt2.getListaGuiaItemDt());
//					
...
//					
//					guiaExecucaoQueixaCrimeNe.salvar(guiaEmissaoDt2, guiaEmissaoDt2.getListaGuiaItemDt(), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
//				}
				...
				if( request.getSession().getAttribute("ListaGuiasRateio") == null && 
					...
					guiaExecucaoQueixaCrimeNe.salvar(guiaEmissaoDt, guiaEmissaoDt.getListaGuiaItemDt(), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
				}
				else {
					...
					if( listaGuiasRateio != null && listaGuiasRateio.size() > 0 ) {
						for( int i = 0; i < listaGuiasRateio.size(); i++ ) {
							...
							auxGuiaEmissaoDt.setNumeroGuiaCompleto( guiaExecucaoQueixaCrimeNe.getNumeroGuiaCompleto(auxGuiaEmissaoDt.getNumeroGuia()) );
							...
							guiaExecucaoQueixaCrimeNe.salvar(auxGuiaEmissaoDt, (List) listaGuiasRateio.get(i), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
				...
				switch(passoEditar) {
					...
					case Configuracao.Imprimir: {
						...
						byte[] byTemp = guiaExecucaoQueixaCrimeNe.imprimirGuia(ProjudiPropriedades.getInstance().getCaminhoAplicacao() , request.getSession().getAttribute("TotalGuia").toString(), processoDt.getServentia(), guiaEmissaoDt, GuiaTipoDt.ID_FINAL_EXECUCAO_QUEIXA_CRIME, "GUIA FINAL(EXEC.SENTENï¿½A QUEIXA-CRIME)");
						
						String nome="GuiaFinalExecSentencaQueixaCrime_Processo_"+ guiaEmissaoDt.getNumeroProcesso();
			...
			case ( BairroDt.CodigoPermissao * Configuracao.QtdPermissao + Configuracao.Localizar ) : {
				if (request.getParameter("Passo")==null){
					...
					String stTemp="";
					stTemp = guiaExecucaoQueixaCrimeNe.consultarDescricaoBairroJSON(stNomeBusca1, stNomeBusca2, stNomeBusca3, posicaopaginaatual);
			...
			default : {
				...
				if( stId != null ) {
					CustaDt custaDt = guiaExecucaoQueixaCrimeNe.consultarCustaDtPorId(stId);
	...
	protected void AdicioneLocomocao(HttpServletRequest request, String stIdRequestBairro, 
		...
		if( stId != null ) {
			...
			if (OficialSPG == null) {
				...
				BairroDt bairroDt = guiaExecucaoQueixaCrimeNe.consultarBairroId(stId);
