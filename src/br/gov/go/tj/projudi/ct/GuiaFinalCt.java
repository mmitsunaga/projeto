...
import br.gov.go.tj.projudi.dt.GuiaFinalDt;
...
public class GuiaFinalCt extends Controle {

	private static final long serialVersionUID = -2446623657473259715L;
	
	private static final String PAGINA_GUIA_FINAL 			= "/WEB-INF/jsptjgo/GuiaFinal.jsp";
	...
	private static final String NOME_CONTROLE_WEB_XML = "GuiaFinal";
	
	private GuiaFinalNe guiaFinalNe = new GuiaFinalNe();

	@Override
	public int Permissao() {
		return GuiaFinalDt.CodigoPermissao;
	}
	...
	public void executar(HttpServletRequest request, HttpServletResponse response, UsuarioNe UsuarioSessao, int paginaatual, String nomebusca, String posicaopaginaatual) throws Exception, ServletException, IOException {
		...
		if ( processoDt != null ) {
			...
			if( processoDt.getId_Serventia() != null ) {
				isProcessoSegundoGrau = guiaFinalNe.isProcessoSegundoGrau(processoDt.getId_Serventia());
		...
		//********************************************
		//Requests 
		request.setAttribute("tempPrograma" 			, "Guia Final");
		...
		if( processoDt != null && processoDt.getServentiaCodigo() != null ) {
			serventiaDt = guiaFinalNe.consultarServentiaProcesso(processoDt.getServentiaCodigo());
			if( serventiaDt == null ) {
				serventiaDt = guiaFinalNe.consultarIdServentia(processoDt.getId_Serventia());
			}
			if( serventiaDt != null ) {
				comarcaDt = guiaFinalNe.consultarComarca(serventiaDt.getId_Comarca());
		...
		if( request.getParameter("rateioCodigo") != null && ( Funcoes.StringToInt(request.getParameter("rateioCodigo")) == GuiaFinalNe.RATEIO_VARIAVEL || Funcoes.StringToInt(request.getParameter("rateioCodigo")) == GuiaFinalNe.RATEIO_50_50 ) ) {
		...
		if( guiaEmissaoDt.getRateioCodigo() != null && guiaEmissaoDt.getRateioCodigo().length() > 0 && ( Funcoes.StringToInt(guiaEmissaoDt.getRateioCodigo()) == GuiaFinalNe.RATEIO_VARIAVEL || Funcoes.StringToInt(guiaEmissaoDt.getRateioCodigo()) == GuiaFinalNe.RATEIO_50_50 ) ) {
		...
		switch(paginaatual) {
			...
			case Configuracao.Novo: {
				...
				if( guiaFinalNe.isServentiaSegundoGrau(processoDt.getId_Serventia()) ) {
				...
				if( !guiaFinalNe.isProcessoSegundoGrau(serventiaDt.getId()) ) {
					...
					if( conectouSPG ) {
						...
						request.getSession().removeAttribute("ListaGuiasParceladasAguardandoPagamento");
						
						//Tratamento para processos sigilosos e segredo de justiça
						guiaFinalNe.tratamentoParaProcessosSigilosoSegredoJustica(processoDt);
						...
						request.getSession().setAttribute("ListaCustaDt", guiaFinalNe.consultarItensGuia(null, GuiaTipoDt.ID_FINAL, processoDt.getId_ProcessoTipo()));
						...
						guiaEmissaoDt.setDataBaseFinalAtualizacao(database.getDataFormatadaddMMyyyy());
						guiaEmissaoDt.setNovoValorAcao( processoDt.getValor() );
						guiaEmissaoDt.setNovoValorAcaoAtualizado( Funcoes.FormatarDecimal(guiaFinalNe.atualizarValorCausaUFR(processoDt.getValor(), Funcoes.StringToDate(processoDt.getDataRecebimento())).toString()) );
						
						guiaEmissaoDt.setRateioCodigo(null);
						guiaEmissaoDt.setContadorQuantidadeAcrescimo("1");
						guiaEmissaoDt.setAtosEscrivaesCivel("1");
						guiaEmissaoDt.setPorcentagemEscrivaesCivel("0");
						if( processoDt != null && processoDt.isCriminal() ) {
							guiaEmissaoDt.setAtoEscrivao("100");
						}
						
						if( guiaFinalNe.isProcessoTipoExecucaoFiscal(processoDt.getProcessoTipoCodigo()) ) {
						...
						if( guiaFinalNe.existeGuiaEmitidaMesmoTipoNaoCancelada(processoDt.getId(), GuiaTipoDt.ID_FINAL) ) {
			...
			case Configuracao.Atualizar : {
				...
				guiaEmissaoDt.setNovoValorAcaoAtualizado( Funcoes.FormatarDecimal(guiaFinalNe.atualizarValorCausaUFR(guiaEmissaoDt.getNovoValorAcao(), Funcoes.StringToDate(guiaEmissaoDt.getDataBaseFinalAtualizacao())).toString()) );
				
				break;
			}
			
			//Apresenta Previsão de Cálculo
			case Configuracao.Curinga6 : {
				switch(passoEditar) {
					...
					case Configuracao.Mensagem : {
						...
						//Atenção!
						//Não tem break pq precisa mostrar a tela novamente de Previsão.
					}
					
					//Utilizado na Volta da Previsão para a Página da Guia
					...
					case Configuracao.Curinga8: {
						...
						guiaFinalNe.validarSomaRateioPartes(request, guiaEmissaoDt, listaGuiasRateio, listaTotalGuiaRateio, listaNomeParteGuia, listaNomePartePorcentagemGuia, listaPromoventes, listaPromovidos);

						
						listaItensGuia = null;
						
						if( listaItensGuia == null ) {
							GuiaModeloDt guiaModeloDt = new GuiaModeloNe().consultarGuiaModeloProcessoTipo(null, GuiaTipoDt.ID_FINAL, processoDt.getId_ProcessoTipo());
							guiaEmissaoDt.setGuiaModeloDt(guiaModeloDt);
						}
						
						if( listaCustaDt != null && listaCustaDt.size() > 0 ) {
							List<GuiaCustaModeloDt> listaAux = (List<GuiaCustaModeloDt>) guiaFinalNe.consultarItensGuiaCustaDt(guiaEmissaoDt, listaCustaDt);
							if( listaAux != null ) {
								if( listaItensGuia == null )
									listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
						...
						if( listaBairroDt != null && listaBairroDt.size() > 0 ) {
							...
							for(int i = 0; i < listaBairroDt.size(); i++) {
								...
								for(int j = 0; j < quantidadeLocomocao; j++) {
									listaItensGuia.add(guiaFinalNe.adicionarItemLocomocaoOficial(bairroLocomocaoDt.getBairroDt(), bairroLocomocaoDt.getOficialSPGDt()));
								}
							}
						}
						...
						if( listaBairroLocomocaoContaVinculada != null && listaBairroLocomocaoContaVinculada.size() > 0 ) {
							...
							for(int i = 0; i < listaBairroLocomocaoContaVinculada.size(); i++) {
								...
								for(int j = 0; j < quantidadeLocomocao; j++) {
									valoresIdBairroContaVinculada.add(bairroLocomocaoDt.getId());
								}
								...
								for(int j = 0; j < quantidadeLocomocao; j++) {
									listaItensGuia.add( guiaFinalNe.adicionarItemLocomocaoOficialContaVinculada(bairroLocomocaoDt.getBairroDt(), bairroLocomocaoDt.getOficialSPGDt()) );
								}
							}
						}
						...
						if( listaItensGuia == null )
							listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
						guiaFinalNe.adicionarItemCalculo(guiaEmissaoDt, listaItensGuia);
						...
						if( guiaEmissaoDt.getContadorQuantidade() != null && guiaEmissaoDt.getContadorQuantidade().length() > 0 && !guiaEmissaoDt.getContadorQuantidade().equals("0") ) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.CONTADOR) );
						}
						...
						if( guiaEmissaoDt.getContadorQuantidadeAcrescimo() != null && guiaEmissaoDt.getContadorQuantidadeAcrescimo().length() > 0 && !guiaEmissaoDt.getContadorQuantidadeAcrescimo().equals("0") ) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.PARA_REALIZACAO_E_CONFERENCIA_DE_CALCULOS_E_ATRIBUICOES_10_MINIMO_ITEM_5) );
						}
						
						
						//Atos dos escrivães do civel
						if( guiaEmissaoDt.getAtosEscrivaesCivel() != null && guiaEmissaoDt.getAtosEscrivaesCivel().length() > 0 && !guiaEmissaoDt.getAtosEscrivaesCivel().equals("0") ) {
							...
							guiaFinalNe.adicionarItemCautelarContencioso(listaItensGuia, guiaEmissaoDt, processoDt);
						}
						...
						if( guiaEmissaoDt.getAtoEscrivao() != null && guiaEmissaoDt.getAtoEscrivao().length() > 0 && !guiaEmissaoDt.getAtoEscrivao().equals("0") ) {
							if( listaItensGuia == null )
								listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
							
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.AUTUACAO_E_OU_PROCESSAMENTO_DE_FEITOS) );
							
							valoresReferenciaCalculo.put(CustaDt.PROCESSO_PROCEDIMENTO_ORDINARIO, guiaEmissaoDt.getAtoEscrivao());
						}
						...
						if( guiaEmissaoDt.getDistribuidorQuantidade() != null && guiaEmissaoDt.getDistribuidorQuantidade().length() > 0 && !guiaEmissaoDt.getDistribuidorQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.ATOS_DE_DISTRIBUICAO_DOS_PROCESSOS_FISICOS_APLICA_SE_10_SOBRE_VALOR_MINIMO_ITEM_5I) );
						}
						
						
						//Taxa Protocolo
						//guiaEmissaoDt.setTaxaProtocoloQuantidade( String.valueOf(guiaFinalNe.consultarInterlocutoriasAdvogado(processoDt.getId())) );
						if( guiaEmissaoDt.getTaxaProtocoloQuantidade() != null && guiaEmissaoDt.getTaxaProtocoloQuantidade().length() > 0 && !guiaEmissaoDt.getTaxaProtocoloQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.TAXA_PROTOCOLO) );
						}
						...
						if( guiaEmissaoDt.getTaxaServicoDividaAtivaQuantidade() != null && guiaEmissaoDt.getTaxaServicoDividaAtivaQuantidade().length() > 0 && !guiaEmissaoDt.getTaxaServicoDividaAtivaQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.TAXA_SERVICO_DIVIDA_ATIVA) );
						}
						
						
						//Atualização valor nominal
						if( guiaEmissaoDt.getAtualizacaoValorNominalQuantidade() != null && guiaEmissaoDt.getAtualizacaoValorNominalQuantidade().length() > 0 && !guiaEmissaoDt.getAtualizacaoValorNominalQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.ATUALIZACAO_MOEDA_NACIONAL) );
						}
...
//								listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
//							
//							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.DESPESA_POSTAL) );
//						}
						...
						if( guiaEmissaoDt.getDepositarioPublico() != null && guiaEmissaoDt.getDepositarioPublico().length() > 0 && !guiaEmissaoDt.getDepositarioPublico().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.DEPOSITO_COMPREENDENDO_OS_REGISTROS_GUARDA_ESCRITURACAO) );
						}
...
//								listaItensGuia = new ArrayList<GuiaCustaModeloDt>();
//							
//							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.POR_DOCUMENTO_PUBLICADO) );
//						}
						...
						if( guiaEmissaoDt.getAvaliadorQuantidade() != null && guiaEmissaoDt.getAvaliadorQuantidade().length() > 0 && guiaEmissaoDt.getAvaliadorValor().length() > 0 && !guiaEmissaoDt.getAvaliadorQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.AUTO_DE_AVALIACAO_DE_BENS_EM_PROCESSO_DE_QUALQUER_NATUREZA) );
						}
						...
						if( guiaEmissaoDt.getLeilaoQuantidade().length() > 0 && guiaEmissaoDt.getLeilaoValor().length() > 0 && !guiaEmissaoDt.getLeilaoQuantidade().equals("0")) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.PREGAO_PRACAO_LEILAO) );
						}
						...
						if( guiaEmissaoDt.getPartidorQuantidade().length() > 0 && !guiaEmissaoDt.getPartidorQuantidade().equals("0") ) {
							...
							listaItensGuia.add( guiaFinalNe.adicionarItem(CustaDt.CUSTA_PARTIDOR) );
						}
						...
						listaGuiaItemDt = guiaFinalNe.calcularItensGuia(guiaEmissaoDt, listaItensGuia, valoresReferenciaCalculo, null, null);
						...
						if( listaGuiaItemDt != null &&
							...
							for( int i = 0; i < listaNomeParteGuia.size(); i++ ) {
								List listaRateioGuiaItemDt = guiaFinalNe.calcularItensGuiaRateio(guiaEmissaoDt, listaGuiaItemDt, Funcoes.StringToDouble(listaNomePartePorcentagemGuia.get(i).toString()));
							...
							if( listaGuiasRateio.size() > 0 ) {
								for( int m = 0; m < listaGuiasRateio.size(); m++ ) {
									listaTotalGuiaRateio.add(guiaFinalNe.calcularTotalGuia((List)listaGuiasRateio.get(m)));
								}
...
//						//Consulta parcelas aguardando pagamento
//						if( guiaEmissaoDt.getId_Processo() != null ) {
//							List<GuiaEmissaoDt> listaGuiasParceladasAguardandoPagamento = guiaFinalNe.consultarGuiaParceladaProcessoAguardandoPagamento(guiaEmissaoDt.getId_Processo());
...
//								for(GuiaEmissaoDt guiaEmissaoTemp: listaGuiasParceladasAguardandoPagamento) {
//									guiaEmissaoTemp.setListaGuiaItemDt(new GuiaItemNe().consultarItensGuia(guiaEmissaoTemp.getId()));
//								}
//								
//								request.setAttribute("ListaGuiasParceladasAguardandoPagamento", listaGuiasParceladasAguardandoPagamento);
//							}
//							
//						}
						
						//Alteração a pedido do Marcelo Tiago por telefone.(Adicionando agora as guias complementares e de serviço em aberto)
						//Consulta guias parcelas aguardando pagamento, guias servicos e complementares em aberto
						if( guiaEmissaoDt.getId_Processo() != null ) {
							List<GuiaEmissaoDt> listaGuiasAdicionarCalculo = null;
							List<GuiaEmissaoDt> listaGuiasServicosAguardandoPagamento = guiaFinalNe.consultarGuiasServicosAguardandoPagamentoProcesso(guiaEmissaoDt.getId_Processo());
							
							if( listaGuiasServicosAguardandoPagamento != null ) {
								listaGuiasAdicionarCalculo = new ArrayList<GuiaEmissaoDt>();
								listaGuiasAdicionarCalculo.addAll(listaGuiasServicosAguardandoPagamento);
							}
							
							List<GuiaEmissaoDt> listaGuiasComplementaresAguardandoPagamento = guiaFinalNe.consultarGuiasComplementaresAguardandoPagamentoProcesso(guiaEmissaoDt.getId_Processo());
							
							if( listaGuiasComplementaresAguardandoPagamento != null ) {
								if( listaGuiasAdicionarCalculo == null ) {
									listaGuiasAdicionarCalculo = new ArrayList<GuiaEmissaoDt>();
								}
								listaGuiasAdicionarCalculo.addAll(listaGuiasComplementaresAguardandoPagamento);
							}
							
							List<GuiaEmissaoDt> listaGuiasParceladasAguardandoPagamento = guiaFinalNe.consultarGuiaParceladaProcessoAguardandoPagamento(guiaEmissaoDt.getId_Processo());
							if( listaGuiasParceladasAguardandoPagamento != null ) {
								if( listaGuiasAdicionarCalculo == null ) {
									listaGuiasAdicionarCalculo = new ArrayList<GuiaEmissaoDt>();
								}
								listaGuiasAdicionarCalculo.addAll(listaGuiasParceladasAguardandoPagamento);
							}
							
							List<GuiaEmissaoDt> listaGuiasGRSGenericaAguardandoPagamento = guiaFinalNe.consultarGuiaGRSGenericaProcessoAguardandoPagamento(guiaEmissaoDt.getId_Processo());
							if( listaGuiasGRSGenericaAguardandoPagamento != null ) {
								if( listaGuiasAdicionarCalculo == null ) {
									listaGuiasAdicionarCalculo = new ArrayList<GuiaEmissaoDt>();
								}
								listaGuiasAdicionarCalculo.addAll(listaGuiasGRSGenericaAguardandoPagamento);
							}
							
							if( listaGuiasAdicionarCalculo != null && !listaGuiasAdicionarCalculo.isEmpty() ) {
								List<GuiaEmissaoDt> listaTemp = new ArrayList<GuiaEmissaoDt>();
								for(GuiaEmissaoDt guiaEmissaoTemp: listaGuiasAdicionarCalculo) {
									guiaEmissaoTemp.setListaGuiaItemDt(new GuiaItemNe().consultarItensGuia(guiaEmissaoTemp.getId()));
								}
							}
							
							if( listaGuiasAdicionarCalculo != null ) {
								request.setAttribute("ListaGuiasParceladasAguardandoPagamento", listaGuiasAdicionarCalculo);
							}
						}
						...
						if( listaGuiaItemDt != null && listaGuiaItemDt.size() > 0 ) {
							...
							if( apresentaPrevia ) {
								...
								request.setAttribute("TotalGuia", guiaFinalNe.getGuiaCalculoNe().getTotalGuia() );
								
								request.getSession().setAttribute("ListaGuiaItemDt" 			, listaGuiaItemDt);
								request.getSession().setAttribute("TotalGuia" 					, Funcoes.FormatarDecimal( guiaFinalNe.getGuiaCalculoNe().getTotalGuia().toString() ) );
								request.setAttribute("visualizarBotaoImpressaoBotaoPagamento"	, guiaFinalNe.visualizarBotaoImpressaoBotaoPagamento(guiaEmissaoDt));
...
//					guiaEmissaoDt.setNumeroGuiaCompleto( guiaFinalNe.getNumeroGuiaCompleto(guiaEmissaoDt.getNumeroGuia()) );
//				}
...
//					guiaEmissaoDt2.setNumeroGuiaCompleto( guiaFinalNe.getNumeroGuiaCompleto(guiaEmissaoDt2.getNumeroGuia()) );
//					
//					guiaFinalNe.calcularRateio50Porcento(guiaEmissaoDt.getListaGuiaItemDt());
//					guiaFinalNe.calcularRateio50Porcento(guiaEmissaoDt2.getListaGuiaItemDt());
//					
...
//					
//					guiaFinalNe.salvar(guiaEmissaoDt2, guiaEmissaoDt2.getListaGuiaItemDt(), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
//				}
				...
				if( request.getSession().getAttribute("ListaGuiasRateio") == null && 
					...
					guiaFinalNe.salvar(guiaEmissaoDt, guiaEmissaoDt.getListaGuiaItemDt(), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
				}
				else {
					...
					if( listaGuiasRateio != null && listaGuiasRateio.size() > 0 ) {
						for( int i = 0; i < listaGuiasRateio.size(); i++ ) {
							...
							auxGuiaEmissaoDt.setNumeroGuiaCompleto( guiaFinalNe.getNumeroGuiaCompleto(auxGuiaEmissaoDt.getNumeroGuia()) );
							...
							guiaFinalNe.salvar(auxGuiaEmissaoDt, (List) listaGuiasRateio.get(i), true, UsuarioSessao.getUsuarioDt().getId_UsuarioServentia());
				...
				switch(passoEditar) {
					...
					case Configuracao.Imprimir: {
						...
						//Geração da guia PDF
						byte[] byTemp = guiaFinalNe.imprimirGuia(ProjudiPropriedades.getInstance().getCaminhoAplicacao() , request.getSession().getAttribute("TotalGuia").toString(), processoDt.getServentia(), guiaEmissaoDt, GuiaTipoDt.ID_FINAL, "FINAL");
						
						String nome="GuiaFinal_Processo_"+ guiaEmissaoDt.getNumeroProcesso();
			...
			case BairroDt.CodigoPermissao * Configuracao.QtdPermissao + Configuracao.Localizar: {
				if (request.getParameter("Passo")==null){
					...
					String stTemp="";
					stTemp = guiaFinalNe.consultarDescricaoBairroJSON(stNomeBusca1, stNomeBusca2, stNomeBusca3, posicaopaginaatual);
			...
			default : {
				...
				if( stId != null ) {
					CustaDt custaDt = guiaFinalNe.consultarCustaDtPorId(stId);
	...
	protected void AdicioneLocomocao(HttpServletRequest request, String stIdRequestBairro, 
		...
		if( stId != null ) {
			...
			if (OficialSPG == null) {
				...
				BairroDt bairroDt = guiaFinalNe.consultarBairroId(stId);
	...
	/**
	 * Método para verificar se guia deve adicionar DUAM ou Não.
	 * @return
	 */
