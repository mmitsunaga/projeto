...
<jsp:useBean id="ServidorJudiciariodt" class= "br.gov.go.tj.projudi.dt.UsuarioDt" scope="session"/>
...
<head>
	...
	<div id="divCorpo" class="divCorpo">
		<div class="area"><h2>&raquo; Log de Usuário </h2></div>
		<form action="ServidorJudiciario" method="post" name="Formulario" id="Formulario">
		
			<input id="PaginaAtual" name="PaginaAtual" type="hidden" value="<%=request.getAttribute("PaginaAtual")%>" />
			<div id="divEditar" class="divEditar">
				<fieldset class="formEdicao">
					<legend>Alterações no Usuário</legend>
					
					<label class="formEdicaoLabel">Usuário</label><br/>
					
					<span class="spanDestaque"><a href="ServidorJudiciario?Id_Usuario=<%=ServidorJudiciariodt.getId()%>"><%=ServidorJudiciariodt.getNome()%></a></span>
					...
					<div id="divBotoesCentralizados" class="divBotoesCentralizados">
						<input name="imgSubmeter" type="submit" value="Buscar" onclick="AlterarValue('PaginaAtual','<%=String.valueOf(Configuracao.Curinga6)%>')" />							
					</div>
															...
															if(listaAtributoLogDt != null){
																...
																if(listaAtributoLogDt.size() > 0){
																	...
																	for(int j = 0; j < listaAtributoLogDt.size(); j++){
																		...
																		if(atributoLogDt.isDiferente()){
																			%>
																			<tr class="TabelaLinha<%= (boLinha?1:2) %>">
																				<td><%= atributoLogDt.getNomeCampo() %></td>
																				<td><font color="FF0000"><%= atributoLogDt.getValorAntigo() %></font></td>
																				<td colspan="3"><font color="FF0000"><%= atributoLogDt.getValorNovo() %></font></td>
																			</tr>
																			<%
																			boLinha = !boLinha;
																		}																	
