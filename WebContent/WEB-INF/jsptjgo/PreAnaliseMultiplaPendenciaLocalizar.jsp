...
<head>
	...
	<title> |<%=request.getAttribute("tempPrograma")%>| Busca de Pré-Análises Múltiplas  </title>
							...
							<tr align="center" class="<%=estiloLinha%>">
								...
								<% 	if (request.getAttribute("podeAnalisar").toString().equalsIgnoreCase("true")){ %>
		                   		<td rowspan="<%=contPendencias%>">
		                   			<a href="AnalisarPendencia?PaginaAtual=<%=Configuracao.Curinga6%>&Id_Arquivo=<%=objTemp.getArquivoDt().getId()%>&preAnalise=true">Analisar</a>     
		                   		</td>
		                   		<% } %>
		                   		<td rowspan="<%=contPendencias%>">
		                   			<a href="PreAnalisarPendencia?PaginaAtual=<%=Configuracao.Curinga8%>&Id_Arquivo=<%=objTemp.getArquivoDt().getId()%>">Refazer</a>     
		                   		</td>
		                   		<td rowspan="<%=contPendencias%>">
		                   			<a href="PreAnalisarPendencia?PaginaAtual=<%=Configuracao.Excluir%>&Id_Arquivo=<%=objTemp.getArquivoDt().getId()%>">Descartar</a>     
		                   		</td>		                   				                   		
	               			</tr>
						<%
								} else { 
