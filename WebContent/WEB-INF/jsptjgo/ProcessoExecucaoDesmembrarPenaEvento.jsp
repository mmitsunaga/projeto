			...
			<form action="ProcessoExecucao" method="post" name="Formulario" id="Formulario">
				...
				<div id="divEditar" class="divEditar">
					...
					<fieldset class="formEdicao"><legend class="formEdicaoLegenda">Processo de Execução Penal Destino</legend>
						<fieldset class="formEdicao"><legend class="formEdicaoLegenda">Selecione os Eventos</legend><br>
							<%
								List liTemp = (List) request.getSession().getAttribute("listaEventoProcessoOrigem");
							...
							<div id="divTabela" class="divTabela">
								<table id="Tabela" class="Tabela" width="50%">
									...
									<%
										ProcessoEventoExecucaoDt objTemp = new ProcessoEventoExecucaoDt();
										boolean boLinha = false;
										for (int i = 0; i < liTemp.size(); i++) {
											objTemp = (ProcessoEventoExecucaoDt) liTemp.get(i);
									%>
									<tr class="TabelaLinha<%=(boLinha ? 1 : 2)%>">
										<td align="center"><input type="checkbox" name="ckEventoProcessoOrigem[]" value=<%=objTemp.getId()%>></input></td>
										<td	align="center"><%=objTemp.getEventoExecucaoDt().getEventoExecucao()%></td>
						...
						Não existe Evento cadastrado para o Processo de Origem.
						<%} %>
					    </fieldset>	
					</fieldset>

					<div id="divBotoesCentralizados" class="divBotoesCentralizados">
						<input name="imgInserir" type="submit" value="Desmembrar Pena" onclick="AlterarValue('PaginaAtual','<%=Configuracao.Editar%>');AlterarValue('PassoEditar','20');"> 
				    </div>
				</div>
			</form>
		</div>
