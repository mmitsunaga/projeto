	...
	<body>
		<div id="divCorpo" class="divCorpo" >
	  	...
	  	<div id="divLocalizar" class="divLocalizar" > 
			<form action="RelatorioAnaliticoProcesso" method="post" name="Formulario" id="Formulario">
			...
			<div id="divEditar" class="divEditar">
				...
				<fieldset class="formEdicao"> 
					...
					<input id="Id_ProcessoTipo" name="Id_ProcessoTipo" type="hidden" value="<%=RelatorioAnaliticodt.getId_ProcessoTipo()%>"/>
					<input  class="formEdicaoInputSomenteLeitura" id="ProcessoTipo" name="ProcessoTipo" readonly="true" type="text" size="60" maxlength="60" value="<%=RelatorioAnaliticodt.getProcessoTipo()%>"/>
					<br />
			  			...
			  			for(int i = 0 ; i < liTemp.size();i++) {
							...
							<%}%>
								<td class="Centralizado" onclick="AlterarValue('PaginaAtual','<%=Configuracao.Curinga8%>'); AlterarValue('Id_Processo','<%=objTemp.getIdProcesso()%>'); FormSubmit('Formulario');"><%= objTemp.getMesRelatorio()%>/<%= objTemp.getAnoRelatorio()%></td>
			                   	...
			                   	<td class="Centralizado" onclick="AlterarValue('PaginaAtual','<%=Configuracao.Curinga8%>'); AlterarValue('Id_Processo','<%=objTemp.getIdProcesso()%>'); FormSubmit('Formulario');"><%= objTemp.getProcessoRelatorio()%></td>
