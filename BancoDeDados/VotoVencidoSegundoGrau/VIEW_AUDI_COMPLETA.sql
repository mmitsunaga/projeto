CREATE OR REPLACE VIEW VIEW_AUDI_COMPLETA AS 
  SELECT VUS.ID_AUDI            AS ID_AUDI,
    ...
    NVL(CASE WHEN PTP.ID_PROC_TIPO IN (1235,144,167)
    THEN
      ...
      AND PP.PROC_PARTE_TIPO_CODIGO  = 132
    ...
    AP.ID_RECURSO_INTERNO        AS ID_RECURSO_INTERNO,
  ...
  AND NOT (AP.ID_PROC IS NULL AND NOT AP.CODIGO_TEMP IS NULL)
  UNION ALL
  SELECT VUS.ID_AUDI            AS ID_AUDI,
    VUS.ID_AUDI_TIPO            AS ID_AUDI_TIPO,
    AT.AUDI_TIPO                AS AUDI_TIPO,
    ...
    THEN
      ...
      AND RP.PROC_PARTE_TIPO_CODIGO = 132
      AND RP.ID_PROC                 = AP.ID_PROC
      AND RP.ID_RECURSO              = R.ID_RECURSO
      AND ROWNUM                     = 1
      )
    ELSE
      (SELECT RP.ID_PROC_PARTE
  ...
  AND EXISTS (SELECT 1
               ...
               WHERE RP.ID_RECURSO = R.ID_RECURSO
                AND RP.ID_PROC_TIPO = PTAP.ID_PROC_TIPO)
  AND ID_RECURSO = (SELECT MAX(ID_RECURSO)
                      FROM PROJUDI.RECURSO R2
                     WHERE R2.ID_PROC = R.ID_PROC
                      AND R2.DATA_RETORNO IS NULL)
  UNION ALL
  SELECT VUS.ID_AUDI            AS ID_AUDI,
    VUS.ID_AUDI_TIPO            AS ID_AUDI_TIPO,
    AT.AUDI_TIPO                AS AUDI_TIPO,
    ...
    (SELECT TO_CHAR(PROC_NUMERO_COMPLETO)
       FROM PROJUDI.AUDI_PROC_FISICO APF
      WHERE APF.ID_AUDI_PROC = AP.ID_AUDI_PROC) AS PROC_NUMERO_COMPLETO,
    AP.DATA_MOVI                 AS DATA_MOVI_AUDI_PROC,
    R.ID_RECURSO                 AS ID_RECURSO,
    R.DATA_RETORNO               AS DATA_RETORNO,
    VUS.CODIGO_TEMP              AS CODIGO_TEMP,
    ...
    (SELECT PROMOVENTE
       FROM PROJUDI.AUDI_PROC_FISICO APF
      WHERE APF.ID_AUDI_PROC = AP.ID_AUDI_PROC) AS PRIMEIRO_PROMOVENTE_RECORRENTE,
    (SELECT PROMOVIDO
       FROM PROJUDI.AUDI_PROC_FISICO APF
      WHERE APF.ID_AUDI_PROC = AP.ID_AUDI_PROC) AS PRIMEIRO_PROMOVIDO_RECORRIDO,
    AP.ACORDO                    AS ACORDO,
    AP.VALOR_ACORDO              AS VALOR_ACORDO,
    AP.ID_PROC_TIPO              AS ID_PROC_TIPO_AUDIENCIA,
    PTAP.PROC_TIPO               AS  PROC_TIPO_AUDIENCIA,
  ...
  ON((AP.ID_PROC_TIPO = PTAP.ID_PROC_TIPO)))  
  WHERE ID_RECURSO IS NULL
  AND AP.ID_PROC IS NULL
  AND NOT AP.CODIGO_TEMP IS NULL
