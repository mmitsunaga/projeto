-- Não é possível renderizar a DLL VIEW para o objeto PROJUDI.VIEW_AUDI_COMPLETA_PRIM_GRAU com o DBMS_METADATA tentando um gerador interno.
CREATE VIEW VIEW_AUDI_COMPLETA_PRIM_GRAU
AS SELECT VUS.ID_AUDI            AS ID_AUDI,
    ...
    SR.SERV                      AS SERV_REDATOR,
      ...
      WHERE PP.DATA_BAIXA IS NULL
  	   ...
  	   AND ROWNUM = 1) AS PRIMEIRO_PROMOVENTE_RECORRENTE,
  	(SELECT PP.ID_PROC_PARTE || '__@--' || PP.NOME  || '__@--' || PP.PROC_PARTE_TIPO
  	   FROM VIEW_PROC_PARTE PP
      WHERE PP.DATA_BAIXA IS NULL
  	   AND PP.PROC_PARTE_TIPO_CODIGO IN (0, 6)
  	   AND PP.ID_PROC =  AP.ID_PROC
  	   AND ROWNUM = 1) AS PRIMEIRO_PROMOVIDO_RECORRIDO,
    AP.ACORDO AS ACORDO,
