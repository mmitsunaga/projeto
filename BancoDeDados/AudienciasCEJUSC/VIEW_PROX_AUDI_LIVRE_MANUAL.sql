CREATE OR REPLACE VIEW PROJUDI.VIEW_PROX_AUDI_LIVRE_MANUAL
AS SELECT 
  ...
  WHERE 
    ((AT.AUDI_TIPO_CODIGO <> 12) AND (AP.ID_AUDI_PROC_STATUS = 8) AND (VUS.DATA_MOVI IS NULL) AND (AP.DATA_MOVI IS NULL) AND (AP.ID_PROC IS NULL) AND (AP.CODIGO_TEMP IS NULL)) 
  ORDER BY 
    VUS.DATA_AGENDADA